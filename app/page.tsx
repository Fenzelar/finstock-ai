...
export default async function StockPage({
  ...
  // ✅ เรียกตรงผ่าน Helper Function (ไม่ต้องผ่าน HTTP fetch)
  const data = await fetchStockDetails(params.symbol, range);
      ...
      <div>
        ...
        <div className="text-center py-16 md:py-20 px-4">
          ...
          <p className="text-muted text-sm mt-2">
            อาจเกิดจาก Yahoo Finance ติดสภาวะชั่วคราว หรือสัญลักษณ์หุ้นไม่ถูกต้อง (ลอง AAPL, TSLA, NVDA)
          </p>
        </div>
      </div>
    );
        ...
        <div className="min-w-0">
          ...
          <div className="flex flex-wrap items-baseline gap-2 md:gap-3 mt-1">
            <span className="font-mono text-2xl md:text-3xl tabular-nums">
              ...
              <span className="text-sm md:text-base text-muted">{quote.currency}</span>
            </span>
